...
// ---------------------------------------------------------
const emailjsConfig = {
  publicKey: "gLmr8vXK5hCUYMjqF",
  serviceId: "service_j6yg8vc",
  templateId: "template_626hedm",
  confirmationTemplateId: "template_8i2aiis"
};
const EMAILJS_READY = emailjsConfig.publicKey !== "YOUR_EMAILJS_PUBLIC_KEY" && typeof window.emailjs !== "undefined";
const CONFIRMATION_READY = EMAILJS_READY && emailjsConfig.confirmationTemplateId !== "YOUR_EMAILJS_CONFIRMATION_TEMPLATE_ID";
if (EMAILJS_READY) {
  window.emailjs.init({ publicKey: emailjsConfig.publicKey });
} else {
  console.warn("EmailJS is not configured yet — registration emails to the school won't be sent. See README.md.");
}
if (EMAILJS_READY && !CONFIRMATION_READY){
  console.warn("Confirmation-to-registrant emails aren't configured yet (confirmationTemplateId) — the school still gets notified normally. See README.md.");
}
...
const i18n = {
  ...
  nl: {
    ...
    "hero.factNum2": "Mechelen",
    "hero.fact2": "Nijverheidsstraat 45",
    "hero.factNum3": "Hamazkayin",
    ...
    "foot.portal": "👨‍👩‍👧 Toegang voor ouders/leerkrachten",
  ...
  en: {
    ...
    "foot.portal": "👨‍👩‍👧 Parent/teacher login",
...
function postCardHTML(p){
  ...
  if (p.media_type === "video") media = `<video src="${p.media_url}" controls></video>`;
    ...
    const idm = p.media_url.match(/(?:v=|youtu\.be\/|shorts\/)([\w-]+)/);
    const vid = idm ? idm[1] : "";
...
async function openPostDetail(post){
  ...
  if (post.media_type === "video") media = `<video src="${post.media_url}" controls style="width:100%; border-radius:12px;"></video>`;
    ...
    const idm = post.media_url.match(/(?:v=|youtu\.be\/|shorts\/)([\w-]+)/);
    const vid = idm ? idm[1] : "";
...
async function sendConfirmationEmail(payload){
  if (!CONFIRMATION_READY || !payload.email) return;
  const { subject, message, toEmail } = buildConfirmationEmail(payload);
  try{
    await window.emailjs.send(emailjsConfig.serviceId, emailjsConfig.confirmationTemplateId, {
      subject, message, to_email: toEmail
    });
  }catch(err){
    console.warn("Confirmation email to registrant could not be sent (registration saved regardless):", err.message || err);
  }
}
...
async function renderPortalParentView(){
  ...
  try{
    ...
    const votingAnnouncementIds = (announcements || []).filter(a=>a.requires_vote).map(a=>a.id);
    let myVotes = {};
    if (votingAnnouncementIds.length){
      const { data: myVoteRows, error: myVotesErr } = await supabase
        .from("announcement_votes")
        .select("*")
        .eq("parent_user_id", portalUser.id)
        .in("announcement_id", votingAnnouncementIds);
      if (myVotesErr) throw myVotesErr;
      (myVoteRows || []).forEach(v=>{ myVotes[v.announcement_id] = v.vote; });
    }

    const scheduleById = {};
    allAssignments.forEach(a=>{ if (a.schedule) scheduleById[a.schedule_id] = a.schedule; });

    // Which teacher(s) teach each of these classes, and whether they've
    ...
    (teacherAssigns || []).forEach(t=>{ (teachersBySchedule[t.schedule_id] ||= []).push(t.teacher_user_id); });

    const announcementGroupsHtml = scheduleIds.map(sid=>{
      const sched = scheduleById[sid];
      if (!sched) return "";
      const teacherIds = teachersBySchedule[sid] || [];
      ...
      const teacherDetailsHtml = teacherIds.map(tid=>{
        ...
        const detailLines = contact ? [
          contact.contact_phone ? `📞 ${escapeHtml(contact.contact_phone)}` : "",
          contact.contact_email ? `✉️ ${escapeHtml(contact.contact_email)}` : ""
        ].filter(Boolean) : [];
        return `<div data-teacherdetails="${sid}-${tid}" style="display:none; padding:14px 18px; background:var(--paper-dim); font-size:.85rem; border-bottom:1px dashed var(--line);">
          ${detailLines.length ? detailLines.join("<br>") : `<span class="helper">Ուսուցիչը դեռ չի կիսվել կոնտակտային տվյալներով։</span>`}
        </div>`;
      }).join("");
      const items = (announcements || []).filter(a=>a.schedule_id === sid);
      const body = items.length
        ? items.map(a=>{
            const myVote = myVotes[a.id];
            const voteUi = a.requires_vote ? `
              <div style="margin-top:10px; padding-top:10px; border-top:1px dashed var(--line);">
                <span class="helper" style="font-weight:600; display:block; margin-bottom:8px;">🗳️ Ուսուցիչը խնդրում է Ձեր պատասխանը՝</span>
                <div style="display:flex; gap:8px;" data-votegroup="${a.id}">
                  <button type="button" class="btn ${myVote === "yes" ? "apricot" : "ghost"} small" data-vote="yes" data-annid="${a.id}">✅ Այո</button>
                  <button type="button" class="btn ${myVote === "no" ? "apricot" : "ghost"} small" data-vote="no" data-annid="${a.id}">❌ Ոչ</button>
                </div>
                <div class="form-msg" data-votemsg="${a.id}"></div>
              </div>` : "";
            return `
      ...
      return `
        <div class="course-group">
          <div class="course-group-head">📚 ${escapeHtml(sched.course||"")} <span style="font-weight:400; opacity:.85; font-size:.82rem;">(${timeLabel(sched.start_time)}–${timeLabel(sched.end_time)})</span>${teacherHtml}</div>
          <div class="course-group-body">${teacherDetailsHtml}${body}</div>
        </div>`;
    }).join("");

    content.innerHTML = childSummaryHtml + `<h3 style="margin:22px 0 14px;">📢 Հայտարարություններ</h3>` + announcementGroupsHtml;

    content.querySelectorAll("[data-toggleteacher]").forEach(btn=>{
      btn.addEventListener("click", ()=>{
        const panel = content.querySelector(`[data-teacherdetails="${btn.dataset.toggleteacher}"]`);
    ...
    content.querySelectorAll("[data-vote]").forEach(btn=>{
      btn.addEventListener("click", async ()=>{
        ...
        const msgEl = content.querySelector(`[data-votemsg="${annId}"]`);
        msgEl.className = "form-msg"; msgEl.textContent = "";
        try{
          const { error } = await supabase.from("announcement_votes")
            .upsert({ announcement_id: annId, parent_user_id: portalUser.id, vote, voted_at: new Date().toISOString() }, { onConflict: "announcement_id,parent_user_id" });
          if (error) throw error;
          content.querySelectorAll(`[data-votegroup="${annId}"] [data-vote]`).forEach(b=>{
            b.classList.toggle("apricot", b.dataset.vote === vote);
            b.classList.toggle("ghost", b.dataset.vote !== vote);
          });
          msgEl.textContent = "Ձեր պատասխանն ընդունվեց ✔"; msgEl.classList.add("show","ok");
        }catch(err){
    ...
    // Mark every announcement just shown as read by this parent, so the
    // teacher can see who has (and hasn't) viewed it. Re-marking on every
    // view just refreshes "last seen" — harmless and simpler than tracking
    // whether it was already marked.
    const announcementIds = (announcements || []).map(a=>a.id);
    if (announcementIds.length){
      supabase.from("announcement_reads")
        .upsert(announcementIds.map(id=>({ announcement_id: id, parent_user_id: portalUser.id })), { onConflict: "announcement_id,parent_user_id" })
        .then(({ error })=>{ if (error) console.warn("Could not mark announcements as read:", error.message); });
    }
  }catch(err){
...
async function renderPortalTeacherView(){
  ...
  content.innerHTML = `<p class="helper">Բեռնվում է…</p>`;
  try{
    const { data: myPrefs } = await supabase.from("teacher_contact_prefs").select("*").eq("teacher_user_id", portalUser.id).maybeSingle();
    const contactSettingsHtml = `
      <div class="portal-announcement" style="background:var(--paper-dim); margin-bottom:22px;">
        <h3 style="margin-top:0;">📞 Իմ կոնտակտային տվյալները ծնողների համար</h3>
        <p class="helper" style="margin-bottom:12px;">Ընտրովի է։ Եթե միացնեք, ծնողները կկարողանան տեսնել Ձեր լրացրած հեռախոսահամարը և/կամ էլ. հասցեն՝ սեղմելով Ձեր անվան կամ դասընթացի վրա։ Անջատած վիճակում ոչինչ չի ցուցադրվի։</p>
        <form id="teacherContactForm">
          <label style="display:flex; align-items:center; gap:8px; margin-bottom:12px; cursor:pointer;">
            <input type="checkbox" id="tc_share" style="width:auto;" ${myPrefs?.share_contact ? "checked" : ""}>
            <span>Կիսվել իմ կոնտակտային տվյալներով ծնողների հետ</span>
          </label>
          <div class="field-row">
            <div class="field"><label for="tc_phone">Հեռախոսահամար</label><input id="tc_phone" type="tel" value="${escapeHtml(myPrefs?.contact_phone || "")}"></div>
            <div class="field"><label for="tc_email">Էլ. հասցե</label><input id="tc_email" type="email" value="${escapeHtml(myPrefs?.contact_email || "")}"></div>
          </div>
          <button class="btn blue small" type="submit" style="margin-top:10px;">Պահպանել</button>
          <div class="form-msg" id="teacherContactMsg"></div>
        </form>
      </div>`;

    function wireTeacherContactForm(){
      document.getElementById("teacherContactForm")?.addEventListener("submit", async (e)=>{
        e.preventDefault();
        const msg = document.getElementById("teacherContactMsg");
        msg.className = "form-msg"; msg.textContent = "";
        try{
          const { error } = await supabase.from("teacher_contact_prefs").upsert({
            teacher_user_id: portalUser.id,
            share_contact: document.getElementById("tc_share").checked,
            contact_phone: document.getElementById("tc_phone").value.trim() || null,
            contact_email: document.getElementById("tc_email").value.trim() || null,
    ...
    if (!assigns || !assigns.length){
      content.innerHTML = contactSettingsHtml + `<div class="portal-empty-note">Ձեր հաշիվը դեռ կապակցված չէ որևէ դասի հետ։ Դիմեք դպրոցի ադմինիստրատորին։</div>`;
      wireTeacherContactForm();
      return;
    }
    ...
    // Read receipts: which connected parents have viewed each announcement.
    const announcementIds = (announcements || []).map(a=>a.id);
    let readsByAnnouncement = {};
    let votesByAnnouncement = {};
    if (announcementIds.length){
      ...
      (reads || []).forEach(r=>{ (readsByAnnouncement[r.announcement_id] ||= new Set()).add(r.parent_user_id); });
      ...
      (votes || []).forEach(v=>{ (votesByAnnouncement[v.announcement_id] ||= {})[v.parent_user_id] = v.vote; });
    }

    content.innerHTML = contactSettingsHtml + assigns.map(a=>{
      const sched = a.schedule;
      if (!sched) return "";
      const classRosterForParents = (rosters || []).filter(r=>r.schedule_id === a.schedule_id);
      // Every unique parent connected to a student in this specific class —
      // used to show a per-announcement "seen by" checklist below.
      const classParents = [];
      const seenParentIds = new Set();
      classRosterForParents.forEach(r=>{
        (parentsByReg[r.registration_id] || []).forEach(p=>{
          if (!seenParentIds.has(p.id)){ seenParentIds.add(p.id); classParents.push(p); }
        });
      });

      const items = (announcements || []).filter(x=>x.schedule_id === a.schedule_id);
      const list = items.length ? items.map(x=>{
        const readIds = readsByAnnouncement[x.id] || new Set();
        const seenHtml = classParents.length ? `
          <div style="margin-top:10px; padding-top:10px; border-top:1px dashed var(--line);">
            <span class="helper" style="font-weight:600;">👁️ Ովքեր են տեսել (${classParents.filter(p=>readIds.has(p.id)).length}/${classParents.length})՝</span>
            <div style="margin-top:6px; display:flex; flex-wrap:wrap; gap:6px;">
              ${classParents.map(p=>{
                const seen = readIds.has(p.id);
                return `<span class="status-pill" style="${seen ? "" : "background:var(--paper-dim); color:var(--ink-soft);"}">${seen ? "✅" : "⏳"} ${escapeHtml(portalCleanName(p.name) || p.email || "")}</span>`;
              }).join("")}
            </div>
          </div>` : `<p class="helper" style="margin-top:8px;">Դեռ ոչ մի ծնողի հաշիվ կապակցված չէ այս դասի ուսանողներին։</p>`;
        const votes = votesByAnnouncement[x.id] || {};
        const yesCount = classParents.filter(p=>votes[p.id] === "yes").length;
        const noCount = classParents.filter(p=>votes[p.id] === "no").length;
        const voteHtml = x.requires_vote ? `
          <div style="margin-top:10px; padding-top:10px; border-top:1px dashed var(--line);">
            <span class="helper" style="font-weight:600;">🗳️ Քվեարկություն՝ ✅ Այո ${yesCount} · ❌ Ոչ ${noCount} · ⏳ դեռ ${classParents.length - yesCount - noCount}</span>
            ${classParents.length ? `<div style="margin-top:6px; display:flex; flex-wrap:wrap; gap:6px;">
              ${classParents.map(p=>{
                const v = votes[p.id];
                const label = v === "yes" ? "✅ Այո" : v === "no" ? "❌ Ոչ" : "⏳ դեռ չի քվեարկել";
                return `<span class="status-pill" style="${v ? "" : "background:var(--paper-dim); color:var(--ink-soft);"}">${escapeHtml(portalCleanName(p.name) || p.email || "")} — ${label}</span>`;
              }).join("")}
            </div>` : ""}
          </div>` : "";
        return `
      ...
      }).join("") : `<div class="portal-announcement"><p class="helper">Դեռ հայտարարություններ չկան։</p></div>`;
      ...
      const rosterRows = classRoster.length ? classRoster.map(r=>{
        ...
        const parentText = parents.length
          ? parents.map(p=>`<button type="button" class="btn ghost small" data-toggleparent="${r.registration_id}" style="padding:4px 10px; font-size:.82rem;">${escapeHtml(portalCleanName(p.name) || p.email || "")} ▾</button>`).join(" ")
          : `<span class="helper">Ծնողի հաշիվ դեռ կապակցված չէ</span>`;
        const detailsHtml = parents.length ? `
          <div data-parentdetails="${r.registration_id}" style="display:none; grid-column:1 / -1; margin-top:8px; padding:10px 12px; background:var(--paper-dim); border-radius:10px; font-size:.85rem;">
            <div style="font-weight:600; margin-bottom:6px;">📞 Կապի տվյալներ</div>
            ${parents.map(p=>`<div>${escapeHtml(portalCleanName(p.name) || "Ծնող")} — ${p.email ? escapeHtml(p.email) : "—"}</div>`).join("")}
            ...
            <div class="portal-announcement" style="background:var(--paper-dim);">
              <form data-postform="${a.schedule_id}">
                ...
                <label style="display:flex; align-items:center; gap:8px; margin-top:10px; cursor:pointer;">
                  <input type="checkbox" data-field="requiresVote" style="width:auto;">
                  <span class="helper" style="margin:0;">🗳️ Այս հայտարարությունը պահանջում է Այո/Ոչ քվեարկություն ծնողներից</span>
                </label>
    ...
    content.querySelectorAll("[data-toggleparent]").forEach(btn=>{
      btn.addEventListener("click", ()=>{
        const panel = content.querySelector(`[data-parentdetails="${btn.dataset.toggleparent}"]`);
        if (panel) panel.style.display = panel.style.display === "none" ? "" : "none";
      });
    });
    ...
    content.querySelectorAll("[data-postform]").forEach(form=>{
      form.addEventListener("submit", async (e)=>{
        ...
        const requiresVote = form.querySelector('[data-field="requiresVote"]').checked;
        try{
          const { error } = await supabase.from("course_announcements").insert({
            schedule_id: scheduleId, title, body: body || null, requires_vote: requiresVote,
